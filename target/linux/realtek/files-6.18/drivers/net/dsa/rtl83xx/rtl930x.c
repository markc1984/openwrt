...
void rtl930x_hasivo_f1100wp_poe_led_set(unsigned int id, bool on);
...
void rtl930x_hasivo_f1100wp_poe_led_set(unsigned int id, bool on)
{
	...
	if (id >= ARRAY_SIZE(copper_ports))
		return;

	/* Stock i2c-poe.ko uses software LED mode 7 for PoE-on and 0 for off. */
	rtl930x_led_swctrl_copr_set(copper_ports[id], 2, on);
}
...
static void rtl930x_led_init_hasivo_f1100wp(struct rtl838x_switch_priv *priv)
{
	...
	/*
	 * Stock port_type 22 programs LED_SET0_0 only:
	...
	/*
	 * Stock splits the masks by front-panel media:
	 * SFP ports use FIB, 10GBase-T ports use COPR, all use COMBO.
	 *
	 * The copper LEDs validate the stock LED_SET0 value and COPR path.
	 * Keep the stock FIB mask, but also expose the SFP ports through COPR;
	 * on this board the visible SFP LEDs appear to be driven by that scan
	 * path rather than the plain FIB mask used by the SDK names.
	 */
